...
import com.example.demo.classofGreat.Generate.Chromo;
...
public class Stuff {
    ...
    String mail;//电子邮箱
    String stores;//所属门店
    List<String> loves;//偏好 三种①工作日  ②工作时间范围 如9-18 ③日工作时间 ④无，正常工作
    //loves输入格式：
    //工作日偏好 1、3、4、5、7
    ...
    public Stuff(String name, String pos, String tel, String mail, String stores, ArrayList<String> loves) {
        this.name = name;
        this.pos = pos;
        this.tel = tel;
        this.mail = mail;
        ...
        for (String love : loves) {
            ...
            if (love.charAt(2) == '日') {
                love_arr.set(0, 1);
                for (int i = 6; i < love.length(); i++) {
                    if (love.charAt(i) == '1') love_arr.set(1, 1);//0则check为休息
                    if (love.charAt(i) == '2') love_arr.set(2, 1);//0则check为休息
                    if (love.charAt(i) == '3') love_arr.set(3, 1);//0则check为休息
                    if (love.charAt(i) == '4') love_arr.set(4, 1);//0则check为休息
                    if (love.charAt(i) == '5') love_arr.set(5, 1);//0则check为休息
                    if (love.charAt(i) == '6') love_arr.set(6, 1);//0则check为休息
                    if (love.charAt(i) == '7') love_arr.set(7, 1);//0则check为休息
                }
            }
            //工作时间偏好 10:00-21:00(范围)
            else if (love.charAt(2) == '时') {
                love_arr.set(0, 2);
                char ch1 = love.charAt(7);
                char ch2 = love.charAt(8);
                int i = ch1 - '0';
                int j = ch2 - '0';
                int res = i * 10 + j;
                if (love.charAt(10) == '3') {
                ...
                j = ch2 - '0';
                res = i * 10 + j;
                if (love.charAt(16) == '3') love_arr.set((res - 9) * 2 + 8, 1);
                else love_arr.set((res - 9) * 2 + 7, 1);
            ...
            } else if (love.charAt(2) == '作') {
                love_arr.set(0, 3);
                int z = love.charAt(8) - '0';//不能超过8小时
                love_arr.set(32, z * 2);
            }
        }
