    ...
    public static void stuff_modifyByName(HashMap<String, Stuff> newList) {
        ...
        switch (input) {
            ...
            case 6:
                ...
                int lovesnums=sc.nextInt();
                List<String> modiLoves=new ArrayList<>();
                for(int i=0;i<lovesnums;i++)
                {
                    modiLoves.add(sc.next());
                }
                newList.get(name).setLoves((ArrayList<String>) modiLoves);
                System.out.println("修改成功！");
                break;
            default:
